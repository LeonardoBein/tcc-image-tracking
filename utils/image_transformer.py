...
class ImageTransformer:
    ...
    def __init__(self, image_path, shape: Optional[Tuple[int, int]] = None):
    ...
    def __getRad(self, theta: float, phi: float, gamma: float) -> Tuple[float, float, float]:
        return np.deg2rad(theta), np.deg2rad(phi), np.deg2rad(gamma)


    """ Wrapper of Rotating a Image """
    def rotateAlongAxis(self, theta=0, phi=0, gamma=0, dx=0, dy=0, dz=0):
        
        # Get radius of rotation along 3 axes
        rtheta, rphi, rgamma = self.__getRad(theta, phi, gamma)
        ...
        mat = self.__getM(rtheta, rphi, rgamma, dx, dy, dz)
        
        return cv2.warpPerspective(self.image.copy(), mat, (self.width, self.height))


    """ Get Perspective Projection Matrix """
    def __getM(self, theta, phi, gamma, dx, dy, dz):
